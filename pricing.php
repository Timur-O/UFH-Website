<?php include('ad_inserter.php');?>
<!doctype html>
<html lang="en" prefix="og: http://ogp.me/ns#">
    <head>

        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta charset="utf-8">
        <meta name="description" content="Ultifree Hosting offers a number of great, affordable plans if the free hosting plan just isn't enough for you. Starting at just $0.99, get your powerful plan today!" />

        <title>Pricing & Plans - Ultifree Hosting</title>

        <meta name="twitter:card" content="summary" />
        <meta name="twitter:site" content="@ultifreehosting" />
        <meta property="og:title" content="Pricing - Ultifree Hosting" />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://ultifreehosting.com/pricing" />
        <meta property="og:image" content="https://ultifreehosting.com/images/favicon.png" />
        <meta property="og:description" content="Ultifree Hosting offers a number of great, affordable plans if the free hosting plan just isn't enough for you. Starting at just $1.67, get your powerful plan today!" />

        <script async src="js/blockadblock.js"></script>
        <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,600" rel="stylesheet">
        <link href='https://fonts.googleapis.com/css?family=Merriweather:300,700' rel='stylesheet' type='text/css'>
        <script src="https://kit.fontawesome.com/b2b26cfd37.js"></script>
        <link href="css/style.css" rel="stylesheet">
        <link rel="icon" href="images/favicon.png">
    </head>
    <body>

    <?php include 'nav.php';?>

        <section class="pricing pricingPage" id="pricing">

            <div class="container">

                <h1>Plans & Pricing</h1>

                <ad>
                  <br />
                  <?php insertAds('pric01', true); ?>
                  <br />
                </ad>

                <hr/>

                <div>
                    <h3>Free<span><i class="fa fa-star"></i></span></h3>
                    <div class="price">$0<span class="pricing_month">/Month</span></div>
                    <ul>
                        <li>5GB Disk Space</li>
                        <li><i class="fas fa-infinity"></i> Bandwidth</li>
                        <li>0 Email Accounts</li>
                        <li>400 MySQL Databases</li>
                        <li><i class="fas fa-infinity"></i> Subdomains</li>
                        <li><i class="fas fa-infinity"></i> Addon Domains</li>
                        <li>Limited Servers</li>
                        <li>Free Sub-Domain</li>
                        <li>10 MB / File Limit</li>
                    </ul>
                    <a class="btn yellow smaller" onclick="BeginSignupEvent('Pricing Page', 'Pricing Comparison Panel');" href="https://app.ultifreehosting.com/signup">Get Started<i class="fa fa-angle-right"></i></a>
                </div>

                <div>
                    <h3>Starter Premium<span><i class="fa fa-star"></i><i class="fa fa-star"></i></span></h3>
                    <div class="price">$19.99<span class="pricing_month">/Year</span></div>
                    <ul>
                        <li>5 GB Disk Space</li>
                        <li>250 GB Bandwidth</li>
                        <li>1 Email Account</li>
                        <li>1 MySQL Database</li>
                        <li>1 Addon Domain</li>
                        <li>1 Subdomain</li>
                        <li>Moderate Servers</li>
                        <li>1 Free Domain</li>
                        <li><i class="fas fa-infinity"></i> / File Limit</li>
                    </ul>
                    <a class="btn yellow smaller" onclick="iFastNetEvent('Pricing Page', 'Starter');" href="https://ifastnet.com/portal/aff.php?aff=26864&a=add&pid=78" rel="nofollow">Get Started<i class="fa fa-angle-right"></i></a>
                </div>

                <div>
                    <h3>Super Premium<span><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i></span></h3>
                    <div class="price">$4.99<span class="pricing_month">/Month</span></div>
                    <ul>
                      <li><i class="fas fa-infinity"></i> Disk Space</li>
                      <li>250 GB Bandwidth</li>
                      <li>100 Email Accounts</li>
                      <li>20 MySQL Databases</li>
                      <li>20 Addon Domains</li>
                      <li>20 Subdomains</li>
                      <li>Considerable Servers</li>
                      <li>6 Free Domains</li>
                      <li><i class="fas fa-infinity"></i> / File Limit</li>
                    </ul>
                    <a class="btn yellow smaller" onclick="iFastNetEvent('Pricing Page', 'Super Premium');" href="https://ifastnet.com/portal/aff.php?aff=26864&a=add&pid=1" rel="nofollow">Get Started<i class="fa fa-angle-right"></i></a>
                </div>
                
                <div>
                    <h3>Ultimate Premium<span><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i></span></h3>
                    <div class="price">$7.99<span class="pricing_month">/Month</span></div>
                    <ul>
                        <li><i class="fas fa-infinity"></i> Disk Space</li>
                        <li><i class="fas fa-infinity"></i> Bandwidth</li>
                        <li><i class="fas fa-infinity"></i> Email Account</li>
                        <li><i class="fas fa-infinity"></i> MySQL Database</li>
                        <li><i class="fas fa-infinity"></i> Addon Domains</li>
                        <li><i class="fas fa-infinity"></i> Subdomains</li>
                        <li>Powerful Servers</li>
                        <li>21 Free Domains</li>
                        <li><i class="fas fa-infinity"></i> / File Limit</li>
                    </ul>
                    <a class="btn yellow smaller" onclick="iFastNetEvent('Pricing Page', 'Ultimate Premium');" href="https://ifastnet.com/portal/aff.php?aff=26864&a=add&pid=6" rel="nofollow">Get Started<i class="fa fa-angle-right"></i></a>
                </div>

            </div>
        </section>

        <br />
        <?php insertAds('pric02', false); ?>
        <br />

        <section class="comparison" id="comparison">
          <div class="container">

            <div class="collapsableHeader">
              <h3>Compare Plans</h3>
            </div>

            <div class="tableContainer">
              <table class="comparisonTable">
                <tr>
                  <th></th>
                  <th>Free</th>
                  <th>Starter Premium</th>
                  <th>Super Premium</th>
                  <th>Ultimate Premium</th>
                </tr>
                <tr>
                  <td>Disk Space</td>
                  <td>5 GB</td>
                  <td>5 GB</td>
                  <td><i class="fas fa-infinity"></i></td>
                  <td><i class="fas fa-infinity"></i></td>
                </tr>
                <tr>
                  <td>Monthly Bandwidth</td>
                  <td><i class="fas fa-infinity"></i></td>
                  <td>250 GB</td>
                  <td>250 GB</td>
                  <td><i class="fas fa-infinity"></i></td>
                </tr>
                <tr>
                  <td>Addon Domains</td>
                  <td><i class="fas fa-infinity"></i></td>
                  <td>1</td>
                  <td>20</td>
                  <td><i class="fas fa-infinity"></i></td>
                </tr>
                <tr>
                  <td>Parked Domains</td>
                  <td><i class="fas fa-infinity"></i></td>
                  <td>1</td>
                  <td>20</td>
                  <td><i class="fas fa-infinity"></i></td>
                </tr>
                <tr>
                  <td>Sub-Domains</td>
                  <td><i class="fas fa-infinity"></i></td>
                  <td>1</td>
                  <td>20</td>
                  <td><i class="fas fa-infinity"></i></td>
                </tr>
                <tr>
                  <td>Email Accounts</td>
                  <td>0</td>
                  <td>1</td>
                  <td>100</td>
                  <td><i class="fas fa-infinity"></i></td>
                </tr>
                <tr>
                  <td>MySQL Databases</td>
                  <td>400</td>
                  <td>1</td>
                  <td>20</td>
                  <td><i class="fas fa-infinity"></i></td>
                </tr>
                <tr>
                  <td>FTP Accounts</td>
                  <td>1</td>
                  <td>100</td>
                  <td><i class="fas fa-infinity"></i></td>
                </tr>
                <tr>
                  <td>Free Domain</td>
                  <td>Sub-Domain (website.ultihost.net)</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/> & 5 Bonus Domains </td>
                  <td><img src="images/check.png"/> & 21 Bonus Domains</td>
                </tr>
                <tr>
                  <td>Site Builder</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Site Statistics</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Softaculous Script Installer</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Custom Error Pages</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Cron Jobs</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>PHP Flags Manager</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>IP Address Deny</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>File Size Limit</td>
                  <td>10 MB / File</td>
                  <td><i class="fas fa-infinity"></i></td>
                  <td><i class="fas fa-infinity"></i></td>
                  <td><i class="fas fa-infinity"></i></td>
                </tr>
                <tr>
                  <td>Custom CNAME Records</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Unmetered MySQL space</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>1-Click Cloudflare Enabler</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Cloudflare Railgun</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td colspan="4"><strong>Server Limits</strong></td>
                </tr>
                <tr>
                  <td>MySQL Connections</td>
                  <td>1</td>
                  <td>20</td>
                  <td>20</td>
                  <td>20</td>
                </tr>
                <tr>
                  <td>CPU Allocation</td>
                  <td>Limited</td>
                  <td>50% / 1 CPU Core</td>
                  <td>100% / 1 CPU Core</td>
                  <td>150% / 1.5 CPU Cores</td>
                </tr>
                <tr>
                  <td>Physical RAM</td>
                  <td>Limited</td>
                  <td>512 MB</td>
                  <td>1 GB</td>
                  <td>1 GB</td>
                </tr>
                <tr>
                  <td>EP Processes</td>
                  <td>Limited</td>
                  <td>10</td>
                  <td>28</td>
                </tr>
                <tr>
                  <td>I/O Limit</td>
                  <td>Low</td>
                  <td>Average</td>
                  <td>High</td>
                  <td>Awesome!</td>
                </tr>
                <tr>
                  <td>Inode Limit</td>
                  <td>30,019</td>
                  <td>180,000</td>
                  <td>180,000</td>
                  <td>180,000</td>
                </tr>
                <tr>
                  <td colspan="4"><strong>Scripting</strong></td>
                </tr>
                <tr>
                  <td>PHP</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>MySQL</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>PhpMyAdmin</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Custom CRON Jobs</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Full .htaccess Control</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Remote MySQL</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>PostgreSQL</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Ruby On Rails</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Node.JS</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Python</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>CGI-BIN</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>MIME Types</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td colspan="4"><strong>E-Commerce</strong></td>
                </tr>
                <tr>
                  <td>PHP cart scripts install ability</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Password Protected Folders</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Bandwidth</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>OS Commerce</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>CubeCart</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Zen Cart</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Prestashop</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Magento(1/2)</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>AbanteCart</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>OpenCart</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>TomatoCart</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>BoxBilling</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>WHMCS</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>ClientExec</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Simple Invoices</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Invoice Ninja</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td colspan="4"><strong>Email</strong></td>
                </tr>
                <tr>
                  <td>Webmail</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Email Accounts (you@yourdomain.com)</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Email Forwarders</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>MX Record Entry</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>SPF Records</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>PHP Sendmail</td>
                  <td>Limited, for activation emails only</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>POP3, IAMP, SMTP</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Spam Filter Spam Assassin</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td colspan="4"><strong>Security</strong></td>
                </tr>
                <tr>
                  <td>Automatic Self Signed SSL (https://)</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Install your own SSL certificate</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>24/7 Monitoring</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Firewall Protection</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Hotlink Protection</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>UPS Power Back-up/Back-up</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Generator</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Web Firewall</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>SiteLock</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Leech Protect</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>File Backup &amp; Restore</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td colspan="4"><strong>Our Guarantees</strong></td>
                </tr>
                <tr>
                  <td>No Hidden Charges</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>7 Day Money Back</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>Price Freeze for the next year</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>99.9% Uptime Guarantee</td>
                  <td>Provided, Not Guaranteed</td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td>24/ 7 Technical Support</td>
                  <td><img src="images/cross.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                  <td><img src="images/check.png"/></td>
                </tr>
                <tr>
                  <td></td>
                  <td><a class="inComparison" onclick="BeginSignupEvent('Pricing Page', 'Pricing Comparison Table');" href="https://app.ultifreehosting.com/signup">Get Started <i class="fa fa-angle-right"></i></a></td>
                  <td><a class="inComparison" onclick="iFastNetEvent('Pricing Page', 'Starter');" href="https://ifastnet.com/portal/aff.php?aff=26864&a=add&pid=78" rel="nofollow">Get Started <i class="fa fa-angle-right"></i></a></td>
                  <td><a class="inComparison" onclick="iFastNetEvent('Pricing Page', 'Super Premium');" href="https://ifastnet.com/portal/aff.php?aff=26864&a=add&pid=1" rel="nofollow">Get Started <i class="fa fa-angle-right"></i></a></td>
                  <td><a class="inComparison" onclick="iFastNetEvent('Pricing Page', 'Ultimate Premium');" href="https://ifastnet.com/portal/aff.php?aff=26864&a=add&pid=6" rel="nofollow">Get Started <i class="fa fa-angle-right"></i></a></td>
                </tr>
              </table>
            </div>

            <br />
            <?php insertAds('pric03', false); ?>
            <br />
            
          </div>
        </section>
        
        <div class="placeholder"></div>
        
        <?php include 'footer.php';?>

        <script data-cookieconsent="ignore" src="js/jquery-3.4.1.min.js"></script>
        <script type="text/javascript" src="js/detectmobilebrowser.js"></script>
        <script async data-cookieconsent="ignore" src="js/analytics.js"></script>
        <script data-cookieconsent="ignore" src="js/mobile_nav.js"></script>
        <script src="js/script.js"></script>
    </body>
</html>
